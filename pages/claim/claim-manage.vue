<template>
	<view class="page">
		...
		<view v-for="(module, index) in modules" v-show="activeTabIndex === index" :key="index" class="module">
			<!-- 我的申请右滑可以删除 -->
			<template v-if="module.moduleKey === 'baoxiaoguanliList'">
								...
								<view class="right">
									...
									<view class="info">
										<view class="info-left">
											<text style="margin-right: 5px;">{{ item.approvalContent }}</text>
											<text style="margin-right: 10px;">{{ item.userName }}</text>
											<text>{{ item.approvalSTime }}</text>
										</view>
			...
			<template v-else>
				<!-- 流程监控有查询 -->
				<view v-if="module.moduleKey === 'baoxiaoguanliMonitoring'" class="search">
					<uni-search-bar
						class="search-bar"
						ref="search"
						clearButton="none"
					...
					<view style="display: flex;align-items: center;">
						<!-- <uni-data-picker
							v-if="!isForProject"
							:localdata="claimTypes"
							popup-title="请选择报销说明"
							placeholder="请选择报销说明"
							:is-clear="true"
							@change="onchange"
						></uni-data-picker> -->
						<app-select v-if="claimTypes.length" v-model="module.searchParams.content" :selectData="claimTypes" @select-change="search"></app-select>
					</view>
				</view>
				<view class="list-wrap">
...
export default {
	...
	data() {
		return {
			queryMethodMap: new Map([
				['baoxiaoguanliList', [getMyClaimPageList, getProjectApplyList]],
				['baoxiaoguanliApprove', [getMyWaitPageList, getProjectWaitList]],
				['baoxiaoguanliYiBan', [getMyEndPageList, getProjectDoneList]],
				['baoxiaoguanliMonitoring', [getFlowMonitorPageList, getProjectMonitoringList]]
			]),
			tabItems: [
				{
					name: '申请',
					icon: 'icon-shenqing1',
					permissionKey: 'baoxiaoguanliList'
				},
				{
					name: '待办',
					icon: 'icon-daiban',
					permissionKey: 'baoxiaoguanliApprove',
					markCount: 0
				},
				{
					name: '已办',
					icon: 'icon-yibanrenwu',
					permissionKey: 'baoxiaoguanliYiBan'
				},
				{
					name: '流程监控',
					icon: 'icon-liucheng1',
					permissionKey: 'baoxiaoguanliMonitoring'
				}
			],
			activeTabIndex: 0,
			more: 'more',
			...
			modules: [], // 所有模块的数据集合
			typeMap: {
				baoxiaoguanliList: 0, // 仅查看
				baoxiaoguanliApprove: 1, // 审批
				baoxiaoguanliYiBan: 2, // 修改审批意见
				baoxiaoguanliMonitoring: 0
			},
			options: [
				{
					text: '删除',
	...
	async onLoad({ projectId }) {
		...
		const methodIndex = this.isForProject ? 1 : 0;
		for (let i = 0; i < this.tabItems.length; i++) {
			const item = {
				...
				queryMethod: this.queryMethodMap.get(this.tabItems[i].permissionKey)[methodIndex],
				moduleKey: this.tabItems[i].permissionKey
			};
			if (this.tabItems[i].permissionKey === 'baoxiaoguanliMonitoring') {
				item.searchParams.userId = this.$hasPermission('baoxiaodanpermission', this.$const.OA_PERMISSION) ? null : userId;
			}
		...
		if (this.isForProject) {
			queryAllLevelTwoParamMap().then(res => {
				this.claimTypes = [{ label: '请选择报销用途', value: '' }].concat(
					(res || []).map(v => {
						return {
							label: v.pname,
							value: v.pid
			...
			this.claimTypes = [{ label: '请选择报销用途', value: '' }].concat(this.approvalContentOptions)
			const typesResult = await getExpenseType();
			const arr = [];
			let index = 0;
			Object.keys(typesResult).forEach(key => {
				const itemsObj = typesResult[key];
				const children = [];
				Object.keys(itemsObj).forEach(itemKey => {
					children.push({
						text: itemsObj[itemKey],
						value: itemKey
					});
				});
				arr.push({
			...
			this.claimTypesOld = arr;
		}
		this.fetchData();
		// 监听阅读状态更新事件
		uni.$on(this.$events.UPDATE_CLAIM_STATE, () => {
	...
	methods: {
		...
		fetchData() {
			...
			searchParams.userName = searchParams.creatorName;
			const module = this.modules[this.activeTabIndex];
			const promise = this.isForProject
				? queryMethod(searchParams)
				: queryMethod(searchParams.pageIndex, searchParams.pageSize, searchParams.userId, searchParams.creatorName, searchParams.approvalContent);
			promise.then(res => {
				res.records = res.records || [];
				res.records.forEach(v => {
					v.approvalSTime = dayjs(v.approvalSTime).format('YYYY-MM-DD');
					v.userNameBak = v.userName.substr(0, 2);
					if (!this.isForProject) {
						const temp = this.claimTypes.find(val => val.value === +v.approvalContent)
						if (temp !== undefined) { 
							v.approvalContent = temp.label
						} else {
							v.approvalContent = this.getClaimType(v);
						}
					}
					if (this.isForProject) {
						v.address = v.moneyMIN.toFixed(2) + '元' // 因为项目报销的项目名称都是同一个，所以这里替换成报销金额
						const temp = this.claimTypes.find(val => val.value === +v.approvalContent)
						if (temp) {
							v.approvalContent = temp.label
						}
		...
		getClaimType(row) {
			for (let i = 0; i < this.claimTypesOld.length; i++) {
				for (let j = 0; j < this.claimTypesOld[i].children.length; j++) {
					if (this.claimTypesOld[i].children[j].value === row.approvalContent) {
						return this.claimTypesOld[i].children[j].text;
					}
				}
			}
			return row.approvalContent;
		},
		search() {
			const module = this.modules[this.activeTabIndex];
			module.searchParams.approvalContent = module.searchParams.content
			module.isLoadAll = false;
			module.more = 'more';
			module.isLoadingMore = false;
			module.dataList.length = 0;
		...
		onchange(e) {
			if (e.detail.value === null) {
				this.modules[this.activeTabIndex].searchParams.approvalContent = '';
			} else {
				this.modules[this.activeTabIndex].searchParams.approvalContent = e.detail.value[e.detail.value.length - 1].value;
			}
			this.search();
		},
		// 切换底部选项卡
		...
		goDetail(item, { moduleKey }) {
			...
			uni.navigateTo({
				url: `/pages/claim/claim-detail?approvalId=${item.approvalId}&type=${type}&isForProject=${this.isForProject ? 'true' : ''}`
			});
		},
		gotoForm() {
			uni.navigateTo({
				url: `/pages/claim/claim-form?projectId=${this.projectId}`
			});
		},
		onClick(e, item) {
			this.showModel = true;
