								...
								<view class="right">
									...
									<view class="info">
										<view class="info-left">
											<text style="margin-right: 5px;">{{ item.address }}</text>
...
export default {
	...
	async onLoad() {
		...
		this.fetchData();
		// 监听阅读状态更新事件
		uni.$on(this.$events.UPDATE_SEAL_STATE, () => {
	...
	methods: {
		...
		goDetail(item, { moduleKey }) {
			...
			uni.navigateTo({
				url: `/pages/seal/claim-detail?approvalId=${item.approvalId}&type=${type}`
			});
		},
		gotoForm() {
			uni.navigateTo({
				url: `/pages/seal/claim-form`
			});
		},
		onClick(e, item) {
			this.showModel = true;
