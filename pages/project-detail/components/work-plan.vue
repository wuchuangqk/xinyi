	...
	<view class="tab-page" id="work-daily">
		...
		<scroll-view scroll-x="true" v-for="item in detailData.xmJournalInfoList">
			<view class="scroll-cell">
				<view class="cell">
					<view class="inner">
						<view class="card-title"><text class="left">施工区域(楼面、区域)</text></view>
				...
				<view class="cell">
					<view class="inner">
						<view class="card-title"><text class="left">任务布置(具体到人、物、工具)</text></view>
		...
		<view class="card">
			<view class="card-title"><text class="left">特别说明</text></view>
...
export default {
	...
	created() {
		...
		this.managePermission = this.$hasPermission('gongchenganpai', this.$const.PROJECT_PERMISSION);
		this.journalTime = this.$dayjs().add(1, 'day').format('YYYY-MM-DD')
		this.fetchData();
	},
	methods: {
		fetchData() {
			const tempData = uni.getStorageSync(this.$const.TEMP)
			const promise = tempData === '' ? getInfoByTime(this.projectId, this.journalTime, 1) : getInfoById(tempData)
