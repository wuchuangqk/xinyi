			...
			<view class="error">
				<view v-for="(item, index) in formData.xmJournalErrors" class="item" :style="{ 'margin-left': isEditState ? '-10px' : '0px' }">
					<template v-if="isEditState">
						<uni-easyinput type="text" v-model="item.errorContent" placeholder="请输入特别说明" :inputBorder="false" />
...
export default {
	...
	async onLoad({ projectId }) {
		...
		// 【管理施工日志权限】
		this.isHasPermission = this.$hasPermission('gongchenganpai', this.$const.PROJECT_PERMISSION);
		this.formData.journalTime = this.$dayjs().add(1, 'day').format('YYYY-MM-DD');
		this.fetchData();
		// 监听选择用户
	...
	methods: {
		fetchData() {
			...
			const promise = tempData === '' ? getInfoByTime(this.formData.projectId, this.formData.journalTime, 1) : getInfoById(tempData);
			uni.removeStorageSync(this.$const.TEMP);
			// 考勤人员名单查工程安排日期的前一天
			const dailyTime = this.$dayjs(this.formData.journalTime).subtract(1, 'day');
			Promise.all([
				getUserList(this.formData.projectId), // 获取项目下参与考勤的有哪些人
				getAttendList(this.formData.projectId, dailyTime.format('YYYY-MM')), // 获取指定月份的考勤记录
				promise,
				getParams(89), // 工种(type=89)
				getInfoByTime(this.formData.projectId, dailyTime.format('YYYY-MM-DD'), 0) // 查今日的施工日志，只有填了施工日志才可以填明日安排
			]).then(([userResponse, attendanceResponse, planRes, workTypeRes, dailyRes]) => {
				...
				const users = userResponse.filter(v => {
					return !v.quitTime || this.$dayjs(dailyTime).isBefore(this.$dayjs(v.quitTime));
				});
				// 筛选出指定日期的考勤
				...
				const records = attendanceResponse
					.filter(v => {
						return v.days === dailyTime.format('YYYY-MM-DD');
					})
					.map(v => v.userId);
				// 有考勤记录的
				...
				res.attendanceCount = res.attendanceCount || 0;
				// 明日安排是否有数据
				const planCount = res.xmJournalInfoList.length;
				this.formData = res;
				if (dailyRes.xmJournalInfoList === null) {
					this.content = '昨日施工日志未填写';
					this.showModel = true;
					this.isEditState = false;
				} else {
					// 在有施工日志的情况，如果填写了明日安排，本日填写的数据可以修改(管理员随时都可以编辑)
					if (planCount !== 0) {
		...
		isCanModify() {
			...
			this.$dayjs().add(1, 'day').format('YYYY-MM-DD') === this.formData.journalTime
		},
		handleChange(e) {
		...
		submit() {
			...
			this.formData.journalName = this.projectName + '项目明日工作安排';
		...
		handleConfirm({ ids, names }, item) {
			...
			let count = 0
			this.formData.xmJournalInfoList.forEach(v => {
				count += v.journalUsers.ids.length
			})
			this.formData.attendanceCount = count
		},
		addError() {
