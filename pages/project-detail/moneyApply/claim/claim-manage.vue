...
import { getMoneyApplyList, getMoneyDoneList, getMoneyMonitoringList, getMoneyWaitList, del, getExpenseType, getSystemParams } from '@/api/office/claim.js';
...
export default {
			...
			tabItems: [
				{
					...
					queryMethod: getMoneyApplyList
				},
				{
					name: '待办',
					icon: 'icon-daiban',
					permissionKey: 'baoxiaoguanliApprove',
					queryMethod: getMoneyWaitList,
					markCount: 0
				},
				{
					name: '已办',
					...
					queryMethod: getMoneyDoneList
				},
				{
					name: '流程监控',
					icon: 'icon-liucheng1',
					permissionKey: 'baoxiaoguanliMonitoring',
					queryMethod: getMoneyMonitoringList
				}
			],
			activeTabIndex: 0,
			more: 'more',
	...
	async onLoad({ projectId }) {
		...
		// 获取款项用途
		this.claimTypes = [{label: '请选择款项用途', value: ''}].concat(await getSystemParams('款项用途'))
		this.fetchData();
		// 监听阅读状态更新事件
		uni.$on(this.$events.UPDATE_MONEY_APPLY_STATE, () => {
	...
	methods: {
		...
		fetchData() {
			...
			queryMethod(searchParams).then(res => {
				...
				res.records.forEach(v => {
					...
					v.moneyMIN = v.moneyMIN.toFixed(2)
		...
		getClaimType(row) {
			const temp = this.claimTypes.find(v => v.value === +row.approvalContent)
			return temp ? temp.label : row.approvalContent
		},
		search() {
			const module = this.modules[this.activeTabIndex];
			module.isLoadAll = false;
		...
		goDetail(item, { moduleKey }) {
			...
			uni.navigateTo({
				url: `/pages/project-detail/moneyApply/claim/claim-detail?approvalId=${item.approvalId}&type=${type}`
			});
		},
		gotoForm() {
			uni.navigateTo({
				url: `/pages/project-detail/moneyApply/claim/claim-form?projectId=${this.projectId}`
			});
		},
		onClick(e, item) {
			this.showModel = true;
