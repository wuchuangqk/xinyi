<template>
	<view class="page">
		...
		<view v-for="(module, index) in modules" v-show="activeTabIndex === index" :key="index" class="module">
			<!-- 我的申请右滑可以删除 -->
			<template v-if="module.moduleKey === 'lizhiFlowManagerList'">
				<uni-swipe-action>
					<uni-swipe-action-item v-for="(item, ind) in module.dataList" :key="ind" :right-options="options" @click="e => onClick(e, item)" class="swipe-item">
						<view class="list-item" @click="goDetail(item, module)">
							<view class="left">
								<view class="photo">
									<text>{{ item.jiaBanRenNameBak }}</text>
								</view>
							</view>
							<view class="right">
								<view class="content">
...
import { getMyLeavePageList, getMyEndPageList, getFlowMonitorPageList, getMyWaitPageList, del } from '@/api/office/leave-office.js';
export default {
	components: { IconFont },
	data() {
		return {
			tabItems: [
				{
					name: '申请',
					icon: 'icon-shenqing1',
					permissionKey: 'lizhiFlowManagerList',
					queryMethod: getMyLeavePageList
				},
				{
					name: '待办',
					icon: 'icon-daiban',
					permissionKey: 'lizhiFlowManagerApprove',
				...
				},
				{
					...
					permissionKey: 'lizhiFlowManagerYiBan',
					queryMethod: getMyEndPageList
				},
				{
					name: '流程监控',
					icon: 'icon-liucheng1',
					permissionKey: 'lizhiFlowManagerMonitoring',
			...
			modules: [], // 所有模块的数据集合
			typeMap: {
				'lizhiFlowManagerList': 0, // 仅查看
				'lizhiFlowManagerApprove': 1, // 审批
				'lizhiFlowManagerYiBan': 2, // 修改审批意见
				'lizhiFlowManagerMonitoring': 0
			},
			options: [
				{
					text: '删除',
	...
	onLoad() {
		...
		// 设置菜单权限
		this.tabItems = this.tabItems.filter(v => {
			if (v.permissionKey === 'lizhiFlowManagerList') {
				return true;
			}
			return this.$hasPermission(v.permissionKey, this.$const.OA_PERMISSION);
		});
		for (let i = 0; i < this.tabItems.length; i++) {
			...
			// 流程监控页面判断有没有权限，没有权限只能看到自己的
			if (this.tabItems[i].permissionKey === 'lizhiFlowManagerMonitoring') {
				item.searchParams.userId = this.$hasPermission('lizhi_monitor_permission', this.$const.OA_PERMISSION) ? null : userId;
			}
			this.modules.push(item);
		}
		this.fetchData();
		// 监听阅读状态更新事件
		uni.$on(this.$events.UPDATE_LEAVE_OFFICE_STATE, () => {
	...
	methods: {
		...
		fetchData() {
			...
			const module = this.modules[this.activeTabIndex];
			queryMethod(searchParams.pageIndex, searchParams.pageSize, searchParams.userId).then(res => {
				res.records = res.records || [];
				res.records.forEach(v => {
					v.created = dayjs(v.created).format('YYYY-MM-DD');
					v.jiaBanRenNameBak = v.jiaBanRenName.substr(0, 2);
				});
		...
		goDetail(item, { moduleKey }) {
			...
			uni.navigateTo({
				url: `/pages/leave-office/leave-office-detail?approvalId=${item.approvalId}&signIndex=${item.signIndex}&type=${type}`
			});
		},
		gotoForm() {
			uni.navigateTo({
				url: '/pages/leave-office/leave-office-form'
			});
		},
		onClick(e, item) {
			this.showModel = true;
