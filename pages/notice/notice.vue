<template>
	<view class="page">
		<view class="list-item" v-for="(item, index) in listData" :key="item.noticeId" @click="toDetail(item.noticeId)">
			<view class="row top">
				<text class="item-title">{{ item.noticeTitle }}</text>
				<!-- <view class="state" :class="item.isRead === 0 ? 'un-read' : 'has-read'">{{ +item.isRead ? '已读' : '未读' }}</view> -->
			</view>
...
import { getNoticeList } from '@/api/office/notice.js';
...
export default {
	...
	methods: {
		...
		fetchData() {
			...
			getNoticeList(this.$hasPermission('notice_all_permission', this.$const.OA_PERMISSION) ? 1 : 0, noticeTitle, isRead, startTime, endTime, pageIndex, pageSize).then(res => {
