	...
	<view class="page">
		...
		<view class="card">
			<view class="detail-item">
				...
				<text class="value">{{ detailData.qingJiaRenName }}</text>
			</view>
			<view class="detail-item">
			...
			<view class="detail-item">
				...
				<text class="value">{{ detailData.qjtypeName }}</text>
			</view>
			<view class="detail-item">
				<text class="label">请假天数</text>
				<text class="value">{{ detailData.leaveDays }}</text>
			</view>
			<view class="detail-item">
			...
			<view class="detail-item">
				...
				<text class="value">{{ detailData.approvalETime }}</text>
			</view>
			<view class="detail-item">
				<text class="label">审批状态</text>
				<text class="value">{{ detailData.stateName }}</text>
			</view>
		</view>
		<view class="card">
			<view class="card-title">
				<view class="left"><text>请假原因</text></view>
			</view>
			<view>{{ detailData.approvalContent }}</view>
		</view>
		<view v-if="detailData.fileList && detailData.fileList.length" class="card">
			<view class="card-title">
		...
		<view v-if="type === 1" class="card approval approval1" :class="{ expend: isExpend }" ref="approval">
			...
			<uni-forms ref="form" :modelValue="formData" :rules="rules" :labelWidth="80" :border="true">
				...
				<!--退回没有抄送-->
				<template v-if="isShowCopy && formData.type === '0'">
					<uni-forms-item label="抄送" name="null">
					...
					<view v-if="isApplyUserTemp" class="copy-tip">当流程结束后，系统会自动抄送给申请人所在项目的负责人</view>
				</template>
...
import { getQJinfo, accept, rollback, flowList as getFlowList, returnUserList, updateDone, determine, copyTo } from '@/api/office/leave.js';
...
export default {
	...
	onReady() {
		...
		getQJinfo(approvalId).then(result => {
			const { approvalETime, approvalSTime } = result.result;
			const millisecondDiff = new Date(this.$dayjs(approvalETime).format('YYYY-MM-DD')).getTime() - new Date(this.$dayjs(approvalSTime).format('YYYY-MM-DD')).getTime();
			result.result.leaveDays = (millisecondDiff / 1000 / 60 / 60 / 24 + 1) + '天';
			const res = result.result;
			res.approvalSTime = this.$dayjs(res.approvalSTime).format('YYYY-MM-DD');
			res.approvalETime = this.$dayjs(res.approvalETime).format('YYYY-MM-DD');
			this.detailData = res;
	...
	methods: {
		...
		doConfirm() {
			...
			determine(this.approvalId).then(() => {
				this.isDoneHandle = true;
				// 更新待办
				uni.$emit(this.$events.UPDATE_TODO_STATE);
				uni.hideLoading();
			});
		 ...
		 * 查询申请人是不是临时工
		 */
		getIsTempUser(userId) {
			getById(userId).then(res => {
				// column_2为用户身份字段（2=班组长，3=临时工）
				this.isApplyUserTemp = +res.column_2 === 2 || +res.column_2 === 3;
