								...
								<view class="right">
									...
									<view class="info">
										<view class="info-left">
											<text style="margin-right: 10px;">{{ item.purposeText }}</text>
											<text style="margin-right: 10px;">{{ item.jiaBanRenName }}</text>
			...
			<template v-else>
				...
				<view v-if="module.moduleKey === 'baoxiaoguanliMonitoring'" class="search">
					...
					<view style="display: flex;align-items: center;">
						<app-select v-if="kindOptions.length" v-model="module.searchParams.purpose" :selectData="kindOptions" @select-change="search"></app-select>
					</view>
				</view>
				<view class="list-wrap">
...
export default {
			...
			tabItems: [
				{
					...
					queryMethod: getMyClaimPageList
				},
				{
					name: '待办',
					icon: 'icon-daiban',
	...
	async onLoad() {
		...
		this.fetchData();
		// 监听阅读状态更新事件
		uni.$on(this.$events.UPDATE_CERTIFICATE_STATE, () => {
	...
	methods: {
		...
		fetchData() {
			...
			queryMethod(searchParams).then(res => {
				...
				res.records.forEach(v => {
					...
					v.userNameBak = v.jiaBanRenName.substr(0, 2);
					v.purposeText = matchLabel(v.purpose, this.kindOptions, true);
				});
		...
		goDetail(item, { moduleKey }) {
			...
			uni.navigateTo({
				url: `/pages/certificate/claim-detail?approvalId=${item.approvalId}&type=${type}`
			});
		},
		gotoForm() {
			uni.navigateTo({
				url: `/pages/certificate/claim-form`
			});
		},
		onClick(e, item) {
			this.showModel = true;
