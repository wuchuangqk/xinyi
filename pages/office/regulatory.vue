		...
		<view v-for="(module, index) in modules" v-show="activeTabIndex === index" :key="index" class="module">
			<view class="list-item" v-for="(item, ind) in module.dataList" :key="ind" @click="goDetail(item)">
				<view class="row top">
					...
					<view class="state" :class="item.isRead === 0 ? 'un-read' : 'has-read'">{{ +item.readed ? '已读' : '未读' }}</view>
				</view>
...
import TheSearch from './work-exchange/the-search.vue';
import dayjs from 'dayjs';
import {getAllInfo } from '@/api/office/exchange.js';
export default {
	components: {
		IconFont,
		TheSearch
	...
	onPullDownRefresh() {
		...
		module.searchParams.infoUse = '';
		module.searchParams.infoType = '规章制度';
