<template>
	<view class="page">
		<view class="header">
			<app-select v-if="deptList.length && !isTempUser" style="margin-right: 30px;" v-model="departId" :selectData="deptList" @select-change="deptChange"></app-select>
			<uni-search-bar :radius="100" @confirm="userSearch" @clear="userSearch" cancelButton="false" placeholder="姓名"></uni-search-bar>
		</view>
		<view class="text">
		...
		<view class="list-item" v-for="(item, ind) in dataList" :key="ind">
			<view class="row top">
			...
			<view class="row bottom">
				...
				<text>
					联系电话：
					<text style="color: #1989fa;" @click.stop="callPhone(item.mobilePhone)">{{ item.mobilePhone }}</text>
				</text>
			</view>
		</view>
...
export default {
	...
	onLoad() {
		...
		this.departId = departId; // 临时工只能看到自己所在的部门
		this.userId = userId;
