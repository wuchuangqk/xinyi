	...
	<view class="page">
		...
		<view class="top">
			<uni-search-bar
				...
				@confirm="search"
				@cancel="handleCancel"
				placeholder="搜索项目名称、项目码、负责人、接收人"
			></uni-search-bar>
			<view class="tab">
		...
		<swiper :current="activeTabIndex" @change="swiperChange" style="flex: 1">
			<swiper-item v-for="item in modules">
				<scroll-view scroll-y="true">
					<view v-for="(itemData, index) in item.dataList" class="info-card" :class="{'info-card-state state-handover' : itemData.isCurUser}" @click="toDetail(itemData)">
						...
						<view class="info-card-footer">
							<view class="footer-item">
								...
								<text>负责人:{{ itemData.oldChargeName }}</text>
							</view>
							<view class="footer-item">
								<view class="footer-item-icon"><image class="img" src="/static/img/user.png" mode="aspectFill" alt=""></view>
								<text>接收人:{{ itemData.newChargeName }}</text>
							</view>
						</view>
					</view>
		...
		<view v-if="isCanAdd" class="float-button" @click="gotoForm"><uni-icons type="plusempty" size="22" color="#ffffff"></uni-icons></view>
		
	</view>
</template>

<script>
import { getManagerPageList } from '@/api/project/projectHandover.js';
export default {
	data() {
		return {
			activeTabIndex: 0,
			tabItems: ['未交接', '已交接', '拒绝交接'],
			modules: [],
			queryContent: '',
			userId: null,
			isCanAdd: false, // 是否有添加项目交接的权限
			isCanView: false, // 是否有查看项目交接的权限
		};
	},
	async onLoad() {
		this.userId = uni.getStorageSync(this.$const.USER_INFO).userId;
		this.isCanAdd = this.$hasPermission('tjxmjj', this.$const.PROJECT_PERMISSION)
		this.isCanView = this.$hasPermission('isSelectChange', this.$const.PROJECT_PERMISSION)
		...
		this.fetchData();
		uni.$on(this.$events.UPDATE_HANDOVER_STATE, () => {
			this.search();
		});
	},
	onReady() {},
	...
	methods: {
		...
		loadData(index) {
			const ind = index === undefined ? this.activeTabIndex : index;
			const module = this.modules[ind];
			...
			getManagerPageList(module.searchParams).then(res => {
				...
				res.records.forEach(v => {
					...
					// 未交接，显示当前用户标识
					if (ind === 0) {
						v.isCurUser = v.newChargeId === this.userId;
					}
				});
		...
		toDetail({ handoverId }) {
			...
			uni.navigateTo({
				url: `/pages/office/project-join/handover-detail?handoverId=${handoverId}`
			});
		},
		search() {
			for (let i = 0; i < this.tabItems.length; i++) {
		...
		gotoForm() {
			uni.navigateTo({
				url:'/pages/office/project-join/handover-form'
			})
		}
	}
};
