...
export default function (isPrompt = false) {
	getCurrentNo(versionInfo => {
		uni.request({
			url: `${BASE_URL}/check-version?version=${versionInfo.versionName}`,
			method: 'GET',
			success: (res) => {
				...
				if (res.data && typeof res.data === 'object' && res.data.file) {
					try {
						const options = {
							updateType: 'forcibly',
							downloadUrl: `${BASE_URL}/${res.data.file}`,
							versionInfo: res.data.remark || '',
							versionName: res.data.version
						}
						updatePopup(options, function () {
							getDownload(options);
						});
					} catch (err) {
						console.log(err)
