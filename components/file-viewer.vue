<template>
	<view>
		<uni-swipe-action>
			<uni-swipe-action-item v-for="(item,index) in filesCopy"
				...
				<view @click="clickFile(item)" class="file-item">
					...
					<view v-else>
						<image class="img" mode="aspectFill" :src="'/static/svg/'+(svgMap.get(item.extend) || '_unknow') + '.svg'" alt="">
					</view>
		...
		<neil-modal :show="showModel" title="提示" :content="modelContent" :show-cancel="false"
		...
		methods: {
			...
			extendFiles(fileList) {
				return fileList.map(v => {
					...
					return {
						fileId: v.fileId,
						fileName: v.fileName + '.' + extend,
						extend,
						url: this.setFileUrl(v.webFilePath),
			...
			uploadImg() {
				uni.chooseImage({
					...
					success: (res) => {
						...
						this.doUpload(res.tempFilePaths[0])
					},
					fail: () => {
						/* uni.showToast({
							title: '未知错误',
